...
data_folder = '../data/';  % assumes CUB dataset is already downloaded there
system(sprintf('wget -nc http://cmp.felk.cvut.cz/~toliageo/ext/mom/train_imlist.txt -P %s/CUB_200_2011/', data_folder));
imlist = textread(sprintf('%s/CUB_200_2011/train_imlist.txt', data_folder), '%s\n');
imsize = 512; 

system(sprintf('wget -nc http://www.vlfeat.org/matconvnet/models/imagenet-googlenet-dag.mat -P %s/', data_folder));
net = load(sprintf('%s/imagenet-googlenet-dag.mat', data_folder));
...
for i = 1:numel(imlist)
	I = imread(fullfile(data_folder, '/CUB_200_2011/images/', imlist{i}));
	...
	if ~mod(i,100), fprintf('Processed %d out of %d\n', i, numel(imlist)); end;
end
fprintf('Done with extraction...\n')

fprintf('PCA - learn and apply whitening...\n')
[~, eigvec, eigval, Xm] = yael_pca(V, 512); 
V = single(vecpostproc(apply_whiten(V, Xm, eigvec, eigval, 512)));
fprintf('Done\n');
save(sprintf('%s/googlenet_rmac_ms_pcaw512_CUB.mat', data_folder), 'V')
